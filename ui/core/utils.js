...
class Tool {
  ...
  static unix2date(timestamp) {
    let date;
    if (timestamp == undefined) date = new Date(+new Date());
    ...
    if (ext == "") {
      ...
      return desc
        ? `${desc[1]
            ...
            .replaceAll(".", "")
            .slice()
            .substring(0, 30)}.bipes.${ext}`
        : imp.length
        ? `my_${imp.slice(-1)[0][1]}_project.bipes.${ext}`
        : `my_project.bipes.${ext}`;
    }
  }
  /**Converts RGB to HEX
  ...
  static warningIfTrue(self, criteria) {
    ...
    if (!self.workspace.isDragging || self.workspace.isDragging()) return;

    let warnings = [];
    criteria.forEach((item, index) => {
      if (item[0]()) warnings.push(item[1]);
    });
...
class files {
  constructor(fileList) {
    ...
    this.blocks2Code.Python.onclick = () => {
      this.internalPython();
    };
    this.blocks2Code.XML.onclick = () => {
      this.internalXML();
    };
  }
  /**
   ...
   */
  resize() {
    if (!Code.current.includes("files")) return;
    ...
    switch (Channel["mux"].currentChannel) {
      ...
      case "webbluetooth":
        ...
        let decoderUint8 = new TextDecoder()
          ...
          .replaceAll(/\t/g, "    ");
        UI["progress"].start(
          parseInt(decoderUint8.length / Channel["webserial"].packetSize) + 1
        );
  ...
  listFiles() {
    mux.bufferPush(
      ...
      files.updateTable.bind(this)
    ); //Using ; to trigger only one ">>>"
  }
  /**
   * Execute a program.
  ...
  get_file(src_fname) {
    ...
    switch (Channel["mux"].currentChannel) {
      ...
      case "webbluetooth":
        ...
        mux.bufferPush(
          `with open('${src_fname}', 'rb') as infile:\rwhile True:\rresult = infile.read(32)\rif result == b'':\rbreak\r\blen = sys.stdout.write(result)\r`,
          () => {}
        ); //Includes dummy callback due to '>>> '
        mux.bufferPush("\r\r\r", () => {
  ...
  get_file_webserial_() {
    ...
    if (re.test(Files.received_string)) {
      ...
      if (get_file_data_.length == 2) Files.get_file_data = get_file_data_[1];

      files.update_file_status(
        "Got " +
          Files.get_file_name +
          ", " +
          Files.get_file_data.length +
          " bytes"
      );
      if (!Files.viewOnly)
        saveAs(
    ...
    if (re.test(this.received_string)) {
      ...
      files_.forEach((file) => {
        ...
        if (!/\./.test(file)) {
          ...
          let downloadButton_ = new DOM("span", {
            className: "icon",
            id: "downloadIcon",
            title: `Download file ${file}`,
          }).onclick(this, Files.files_download, [file]);

          let wrapper_ = new DOM("div").append([
            runButton_,
            downloadButton_,
            deleteButton_,
          ]);
  ...
  editedXML2Workspace() {
    ...
    if (result === true) {
      let content = UI["workspace"].readWorkspace(
  ...
  handleCurrentProject() {
    ...
    if (this.file_save_as.className == "bipes-py") this.internalPython();
    else if (Files.file_save_as.className == "bipes-xml") this.internalXML();
  }
}
...
class DOM {
      ...
      case "div":
        ...
        if (typeof tags == "object")
          for (const tag in tags) {
            if (
              ["innerText", "className", "id", "title", "innerText"].includes(
                tag
              )
            )
  ...
  onclick(self, ev, args) {
    this._dom.onclick = () => {
      if (typeof args == "undefined") ev.bind(self)();
      else if (args.constructor == Array) ev.apply(self, args);
    };
    return this;
  }
  /**
   ...
   */
  append(DOMS) {
    if (DOMS.constructor != Array) DOMS = [DOMS];

    DOMS.forEach((item) => {
      if (/HTML(.*)Element/.test(item.constructor.name))
        this._dom.appendChild(item);
      else if (
        item.constructor.name == "DOM" &&
        /HTML(.*)Element/.test(item._dom)
      )
...
class term {
   ...
   */
  static resize() {
    if (!Code.current.includes("console")) return;
    ...
    if (Code.current[0] == "console")
      ...
      cols =
        Math.max(50, Math.min(200, (window.innerWidth / 2 - 4 * $em) / 7)) | 0;

    let rows =
      Math.max(15, Math.min(40, (window.innerHeight - 20 * $em) / 12)) | 0;
