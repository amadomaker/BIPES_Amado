// Generated from css/assets/songs.py
const SONGS = [
...
const SONG_MAP = new Map(SONGS.map((song) => {
  const name = String(song).split(":")[0].trim();
  return [name, song];
}));
const SONG_NAMES = SONGS.map((song) => String(song).split(":")[0].trim());

export function getSongNames() {
  return SONG_NAMES.slice();
}

export function findSongByName(name) {
  if (!name) return null;
  return SONG_MAP.get(String(name)) ?? null;
}
