 ...
 *    ```
 */
let ShowPinsElement = class ShowPinsElement extends LitElement {
    constructor() {
        super(...arguments);
        this.pinColor = 'red';
    ...
    get slotChild() {
        return this.elementSlot?.assignedElements()[0];
    }
    handleSlotChange() {
    ...
    render() {
      ...
      <svg style="position: absolute; top: 0; left: 0" width="100%" height="100%" fill=${pinColor}>
        ${pinInfo.map((pin) => svg `<circle cx=${pin.x} cy=${pin.y} r=2><title>${pin.name}</title></circle>`)}
      </svg>
    </div>`;
    }
};
